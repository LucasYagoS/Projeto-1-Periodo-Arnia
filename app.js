...
const getPosts = async () => {
    const apiResponse = await fetch('https://api-projeto-modulo1.onrender.com/users')
...
const getPacient = async (id) => {
    const apiResponse = await fetch(`https://api-projeto-modulo1.onrender.com/pacients/${id}`)
    const pacient = await apiResponse.json()
    return pacient
}
const getDoc = async (userId) => {
    const apiResponse = await fetch(`https://api-projeto-modulo1.onrender.com/users/${userId}`)
...
const createUser = async (user) => {
    await fetch("https://api-projeto-modulo1.onrender.com/users", {
        method: "POST",
        headers: {
...
const login = async () => {
    ...
    const apiResponse = await fetch(`https://api-projeto-modulo1.onrender.com/users?Email=${userLogin}&Senha=${userPassword}`)
    const users = await apiResponse.json()
    if (users.length > 0) {
...
const listPacients = async () => {
    ...
    const apiResponse = await fetch(`https://api-projeto-modulo1.onrender.com/pacients?docId=${userId}`)
...
const createPacient = async (pacient) => {
    await fetch("https://api-projeto-modulo1.onrender.com/pacients", {
        method: "POST",
        headers: {
            'Accept': 'application/json, text/plain, */*',
            'Content-Type': 'application/json'
...
const delPacient = async (id) => {
    await fetch(`https://api-projeto-modulo1.onrender.com/pacients/${id}`, {
        method: 'DELETE'
    })
    location.reload()
}
...
const filterPacients = async (userId) => {
    ...
    if (searchParam === "") {
        ...
        const apiResponse = await fetch(`https://api-projeto-modulo1.onrender.com/pacients?docId=${userId}&q=${searchParam}`);
